...
StoreAdmin.start = function () {
	$(document).ready(function () {
		...
		StoreAdmin.bindForms();
		StoreAdmin.loadStoreInfo();
		$("#welcome-page").fadeIn();
	});
};
...
StoreAdmin.bindForms = function () {
	...
	var storeSettingsForm = $("form#settings")
	storeSettingsForm.submit(function (e) {
		e.preventDefault();
		var submittedForm = $(this);
		$.post('/settings', submittedForm.serialize(), function (result) {
		}, "json")
	})
};
...
StoreAdmin.loadStoreInfo = function () {
	$.ajax({
		type: "GET",
		url: "/settings",
		dataType: 'json',
